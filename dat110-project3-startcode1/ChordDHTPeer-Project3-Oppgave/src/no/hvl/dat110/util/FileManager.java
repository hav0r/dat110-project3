...
public class FileManager {
    ...
    public int distributeReplicastoPeers() throws RemoteException{
    	...
    	for(BigInteger rep : replicafiles) {
    		NodeInterface successor = chordnode.findSuccessor(rep);
    		successor.addKey(rep);
    		if(counter==index) {
    			successor.saveFileContent(filename, rep, bytesOfFile, true);
    		}else {
    			successor.saveFileContent(filename, rep, bytesOfFile, false);
    		}
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for(BigInteger rep : replicafiles) {
			NodeInterface successor = chordnode.findSuccessor(rep);
			succinfo.add(successor.getFilesMetadata(rep));
		}
