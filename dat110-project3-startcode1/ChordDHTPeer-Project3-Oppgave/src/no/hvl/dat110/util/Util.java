...
public class Util {
	...
	public static boolean computeLogic(BigInteger id, BigInteger lower, BigInteger upper) {
		...
		BigInteger upperCopy = upper;

		if (lower.compareTo(upper) == 1) {// hvis lower > upper
			upperCopy = upper.add(addressSize);

			if (((id.compareTo(new BigInteger("0")) == 0 || id.compareTo(new BigInteger("0")) == 1))
					&& ((id.compareTo(upper) == 0 || id.compareTo(upper) == -1))) {
